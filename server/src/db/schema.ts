...
function createTables(db: Database.Database): void {
  db.exec(`
    ...
    -- Wallet (balance/deposit/withdraw/history/categories) moved to
    -- Firestore — see server/src/firestore/*Repo.ts and
    -- services/walletService.ts. The wallet_accounts/wallet_transactions
    -- tables that used to live here are retired; nothing reads or writes
    -- them anymore. (Any rows from a pre-Firestore install are simply left
    -- alone in the SQLite file — dropping this CREATE statement doesn't
    -- delete existing data, it only stops declaring the tables on fresh installs.)
