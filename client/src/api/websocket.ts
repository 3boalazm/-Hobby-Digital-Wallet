...
function getWsUrl(wsToken: string): string {
  // VITE_WS_URL overrides both protocol+host (e.g. 'wss://api.example.com') for a
  // split deployment; unset means same-origin, unchanged from before.
  const override = import.meta.env.VITE_WS_URL
  if (override) return `${override.replace(/\/+$/, '')}/ws?token=${wsToken}`
...
async function fetchWsToken(): Promise<string | null> {
  try {
    ...
    const resp = await fetch(`${apiBase.replace(/\/+$/, '')}/auth/ws-token`, {
      method: 'POST',
      credentials: 'include',
    })
    if (resp.status === 401) {
